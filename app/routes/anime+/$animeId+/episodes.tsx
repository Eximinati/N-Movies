...
export const meta = mergeMeta<{}, { 'routes/anime+/$animeId': typeof animeIdLoader }>(
  ({ params, matches }) => {
    ...
    return [
      { title: `Nmovies - ${animeTitle} - Episodes` },
      {
        name: 'description',
        content: description
          ? description?.replace(/<\/?[^>]+(>|$)/g, '')
          : `Watch ${animeTitle} in Nmovies`,
      },
      {
        property: 'og:url',
        content: `https://sorachill.vercel.app/anime/${params.animeId}/episodes`,
      },
      { property: 'og:title', content: `Nmovies - ${animeTitle} - Episodes` },
      {
      ...
      { name: 'twitter:title', content: `Nmovies - ${animeTitle} - Episodes` },
      {
