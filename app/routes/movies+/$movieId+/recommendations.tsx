...
export const meta = mergeMeta<typeof loader, { 'routes/movies+/$movieId': typeof movieIdLoader }>(
  ({ params, matches }) => {
    ...
    return [
      { title: `Nmovies - ${movieTitle} - Recommendations` },
      {
        property: 'og:url',
        content: `https://sorachill.vercel.app/movies/${params.movieId}/recommendations`,
      },
      { property: 'og:title', content: `Nmovies - ${movieTitle} - Recommendations` },
      { name: 'twitter:title', content: `Nmovies - ${movieTitle} - Recommendations` },
    ];
  },
);
