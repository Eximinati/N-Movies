...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `Nmovies - ${name} ${seasonDetail?.name || ''} - Crew` },
    {
      property: 'og:url',
      content: `https://sorachill.vercel.app/tv-shows/${params.tvId}/season/${params.seasonId}/crew`,
    },
    { property: 'og:title', content: `Nmovies - ${name} ${seasonDetail?.name || ''} - Crew` },
    { name: 'twitter:title', content: `Nmovies - ${name} ${seasonDetail?.name || ''} - Crew` },
  ];
});
