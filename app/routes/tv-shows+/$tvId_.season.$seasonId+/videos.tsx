...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `Nmovies - ${name} ${seasonDetail?.name || ''} - Videos` },
    {
      property: 'og:url',
      content: `https://sorachill.vercel.app/tv-shows/${params.tvId}/season/${params.seasonId}/videos`,
    },
    { property: 'og:title', content: `Nmovies - ${name} ${seasonDetail?.name || ''} - Videos` },
    { name: 'twitter:title', content: `Nmovies - ${name} ${seasonDetail?.name || ''} - Videos` },
  ];
});
