...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `Nmovies - ${name} ${seasonDetail?.name || ''} - Cast` },
    {
      property: 'og:url',
      content: `https://sorachill.vercel.app/tv-shows/${params.tvId}/season/${params.seasonId}/cast`,
    },
    { property: 'og:title', content: `Nmovies - ${name} ${seasonDetail?.name || ''} - Cast` },
    { name: 'twitter:title', content: `Nmovies - ${name} ${seasonDetail?.name || ''} - Cast` },
  ];
});
