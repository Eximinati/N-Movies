...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `Nmovies - ${name} ${seasonDetail?.name || ''} - Photos` },
    {
      property: 'og:url',
      content: `https://sorachill.vercel.app/tv-shows/${params.tvId}/season/${params.seasonId}/photos`,
    },
    { property: 'og:title', content: `Nmovies - ${name} ${seasonDetail?.name || ''} - Photos` },
    { name: 'twitter:title', content: `Nmovies - ${name} ${seasonDetail?.name || ''} - Photos` },
  ];
});
